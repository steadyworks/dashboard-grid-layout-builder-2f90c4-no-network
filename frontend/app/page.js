...
function defaultConfig(type) {
  switch (type) {
    ...
    case 'progress':   return { percentage: '50', label: 'Progress' }
    default:           return {}
  }
}
...
function ProgressRingContent({ config }) {
  const pct = Math.max(0, Math.min(100, parseFloat(config.percentage) || 0))
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', gap:4 }}>
      <svg width={80} height={80} viewBox="0 0 80 80">
        ...
        />
        <text x={40} y={44} textAnchor="middle" fontSize={13} fontWeight={700} fill="#1a1a2e">{pct}%</text>
      </svg>
...
function WidgetContent({ type, config }) {
  switch (type) {
    ...
    case 'progress':  return <ProgressRingContent config={config} />
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
        ...
        >
          ...
          {[
            ...
            { type:'progress',  testid:'palette-progress',  label:'Progress Ring' },
          ].map(item => (
            <button
              key={item.type}
